class App {
    ...
    setupGlobalEvents() {
        document.addEventListener('click', (e) => {
            ...
            switch(action) {
                case 'goHome': this.currentPage = 'home'; this.selectedLessonId = null; this.isUnlockTest = false; break;
                case 'selLevel': this.selectedLevel = param; this.selectedLessonId = null; this.currentPage = (param === 'custom_list') ? 'custom_lessons_view' : 'lessons'; break;
                case 'selLesson':
                    ...
                    if (isUnlocked) { this.selectedLessonId = param; this.currentPage = 'reading'; } 
                    else {
                ...
                case 'deleteWord': 
                    if(confirm('حذف نهائي؟')) {
                        this.hiddenFromCards.push(String(param)); this.saveData(); 
                        if (this.currentCardIndex > 0) this.currentCardIndex--;
                        this.render();
                    } break;
                case 'masterWord': this.masteredWords.push(param); this.saveData(); if (this.currentCardIndex > 0) this.currentCardIndex--; this.render(); break;
                case 'restartCards': this.currentCardIndex = 0; this.render(); break;
                case 'resetAll': 
                    if(confirm('تصفير الكلمات؟')) {
                ...
                case 'editTitle':
                    const newTitle = prompt('العنوان الجديد:', param);
    ...
    getView(lesson, allTerms) {
        if (this.currentPage === 'home') {
            ...
            return `<main class="main-content">
                ...
                <div class="features-grid">
                    ...
                    ${hasCustom ? `<div class="feature-card" data-action="selLevel" data-param="custom_list" style="background:#fef3c7; border:2px solid #f59e0b;"><h3>📂 الدروس المحفوظة</h3></div>` : ''}
                </div>
            </main>`;
        }
        if (this.currentPage === 'custom_lessons_view') {
            return `<main class="main-content"><button class="hero-btn" data-action="goHome">← الرئيسية</button><div class="features-grid" style="margin-top:10px;">
        ...
        if (this.currentPage === 'lessons') {
            const list = window.lessonsList[this.selectedLevel] || [];
            return `<main class="main-content"><button class="hero-btn" data-action="goHome">← الرئيسية</button><div class="features-grid" style="margin-top:20px;">
                ${list.map(l => {
        ...
        if (this.currentPage === 'reading') {
            ...
            return `<main class="main-content">
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-top:15px;"><h2 style="flex:1;">${lesson.title}</h2>
                ${isCustom ? `<button class="hero-btn" data-action="editTitle" data-param="${lesson.title}" style="background:#64748b; padding:5px 10px;">📝 تعديل</button>` : ''}</div>
                <div class="reading-card" style="margin-top:15px; max-height:350px; overflow-y:auto; border-left:4px solid #1e40af; direction: ltr; text-align: left;">${lesson ? lesson.content : ''}</div>
                <div style="background:white; padding:15px; border-radius:12px; margin-top:20px; box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <button class="hero-btn" data-action="addNewWord" style="width:100%; background:#16a34a;">+ حفظ</button>
                </div>
            </main>`;
        }
        if (this.currentPage === 'quiz') {
            if (this.quizIndex >= this.quizQuestions.length) {
                ...
                if (this.isUnlockTest && s >= 70) { this.unlockedLessons.push(String(this.tempLessonToUnlock)); this.saveData(); }
                return `<main class="main-content" style="text-align:center;"><div class="reading-card"><h1>${s}%</h1><button class="hero-btn" data-action="setPage" data-param="reading">متابعة</button></div></main>`;
            }
            const q = this.quizQuestions[this.quizIndex];
            return `<main class="main-content">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px;"><button class="hero-btn" data-action="setPage" data-param="reading">✕</button><b>${this.quizIndex+1}/${this.quizQuestions.length}</b></div>
                <div class="reading-card" style="text-align:center;"><h1>${q.english}</h1><button class="hero-btn" data-action="speak" data-param="${q.english}">🔊</button>
                <div style="margin-top:30px; display:grid; gap:10px;">${this.quizOptions.map(opt => `<button class="quiz-opt-btn" data-action="ansQ" data-param="${opt}" data-correct="${q.arabic}">${opt}</button>`).join('')}</div></div>
            </main>`;
        }
        if (this.currentPage === 'flashcards') {
            const active = allTerms.filter(t => !this.masteredWords.includes(t.id) && !this.hiddenFromCards.includes(String(t.id)));
            if (active.length === 0) return `<main class="main-content" style="text-align:center;"><div class="reading-card"><h2>🎉 انتهت!</h2><button class="hero-btn" data-action="resetAll" data-param='${JSON.stringify(allTerms.map(x=>x.id))}' style="background:#dc2626;">🔄 تصفير</button></div></main>`;
            const t = active[this.currentCardIndex] || active[0];
            return `<main class="main-content">
                <div style="display:flex; justify-content:space-between; margin-bottom:15px;"><button class="hero-btn" data-action="setPage" data-param="reading">←</button><b>${this.currentCardIndex+1}/${active.length}</b></div>
                <div class="flashcard-container" onclick="this.querySelector('.flashcard').classList.toggle('flipped')">
                    <div class="flashcard">
                        <div class="flashcard-front"><h1>${t.english}</h1><button class="hero-btn" data-action="speak" data-param="${t.english}" style="margin-top:15px; background:rgba(255,255,255,0.2); border:1px solid white;">🔊 نطق</button></div>
                        <div class="flashcard-back"><h1>${t.arabic}</h1></div>
                    </div>
                </div>
                <div style="display:flex; gap:10px;"><button class="hero-btn" data-action="masterWord" data-param="${t.id}" style="background:#059669; flex:1;">✅ حفظت</button><button class="hero-btn" data-action="deleteWord" data-param="${t.id}" style="background:#dc2626; flex:1;">🗑️ حذف</button></div>
                <div style="display:flex; justify-content:center; gap:20px; margin-top:15px;"><button class="hero-btn" data-action="prevC">السابق</button><button class="hero-btn" data-action="nextC" data-total="${active.length}">التالي</button></div>
                <center style="margin-top:20px;"><button class="hero-btn" data-action="restartCards" style="background:#64748b; font-size:0.8rem;">🔄 إعادة الجولة</button></center>
            </main>`;
        }
        if (this.currentPage === 'addLesson') {
            return `<main class="main-content"><button class="hero-btn" data-action="goHome">← رجوع</button>
                <div class="reading-card" style="margin-top:20px;"><h3>📸 الكاميرا</h3>
