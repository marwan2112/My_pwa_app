class App {
    ...
    // --- ميزة الترجمة التلقائية الجديدة ---
    ...
    async handleAutoTranslate(word) {
        const arbInput = document.getElementById('newArb');
        if (!arbInput || arbInput.value.trim() !== "") return; // لا نغير الترجمة إذا كان المستخدم كتب شيئاً يدوياً
            ...
            return `<main class=\"main-content\">
               ...
               <div class=\"reading-card\" style=\"margin-top:15px; max-height:350px; overflow-y:auto; border-left:4px solid #1e40af; direction: ltr; text-align: left;\">${lesson ? lesson.content : 'لا يوجد نص'}</div>
                <div style=\"background:white; padding:15px; border-radius:12px; margin-top:20px; box-shadow:0 2px 8px rgba(0,0,0,0.1);\">
                    ...
                    <input id=\"newEng\" placeholder=\"English\" style=\"width:100%; padding:10px; margin:5px 0; border:1px solid #ddd; border-radius:5px;\" onblur=\"appInstance.handleAutoTranslate(this.value)\">
                    <input id=\"newArb\" placeholder=\"المعنى\" style=\"width:100%; padding:10px; margin-bottom:10px; border:1px solid #ddd; border-radius:5px;\">
