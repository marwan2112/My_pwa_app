class App {
    constructor() {
        // حماية من الشاشة البيضاء: إذا لم تكن البيانات جاهزة، ننتظر قليلاً
        ...
        // استرجاع البيانات المخزنة (دون حذف أي شيء قديم)
    ...
    // إصلاح الصوت ليعمل على جميع المتصفحات
    playSound(type) {
        try {
    ...
    // الحذف من البطاقات فقط (بقاء الكلمة في الاختبار)
    deleteFromCards(id) {
        if (confirm('إزالة من البطاقات؟ ستبقى الكلمة موجودة في الاختبارات.')) {
    ...
    // إعداد الاختبارات (فصل تام)
    prepareQuiz(terms, isUnlockMode = false) {
        this.isUnlockTest = isUnlockMode;
        let pool = [...terms]; // الاختبار دائماً شامل لكل الكلمات حتى المحذوفة من البطاقات
        ...
        document.addEventListener('click', (e) => {
            ...
            switch(action) {
                ...
                case 'addNewWord':
                    ...
                    if(eng && arb) {
                        this.userVocabulary.push({ id: "u"+Date.now(), lessonId: String(this.selectedLevel), english: eng, arabic: arb });
    ...
    getView(lesson, allTerms) {
        if (this.currentPage === 'home') {
            return `<main class="main-content"><button class="hero-btn" data-action="setPage" data-param="addLesson" style="background:#8b5cf6; width:100%;">📸 إضافة نص من الكاميرا</button><div class="features-grid" style="margin-top:20px;">${window.levels.map(l => `<div class="feature-card" data-action="selLevel" data-param="${l.id}"><h3>${l.icon} ${l.name}</h3></div>`).join('')}</div></main>`;
        }
        ...
        if (this.currentPage === 'reading') {
            return `<main class="main-content">
                ...
                <div class="reading-card" style="margin-top:15px; border-top: 5px solid #1e40af;">
                    <div style="max-height:400px; overflow-y:auto; padding-right:10px;">${lesson ? lesson.content : 'لا يوجد نص'}</div>
                </div>
                <div style="background:#f0fdf4; padding:15px; border-radius:12px; margin-top:15px; border:1px solid #bbf7d0;">
                    <input type="text" id="newEng" placeholder="الكلمة بالإنجليزية" style="width:100%; padding:10px; margin-bottom:5px; border-radius:8px; border:1px solid #ccc;">
                    <input type="text" id="newArb" placeholder="المعنى بالعربية" style="width:100%; padding:10px; margin-bottom:5px; border-radius:8px; border:1px solid #ccc;">
                    <button class="hero-btn" data-action="addNewWord" style="background:#16a34a; width:100%;">+ إضافة للدرس</button>
                </div></main>`;
        }
        ...
        if (this.currentPage === 'quiz') {
            if (this.quizIndex >= this.quizQuestions.length) {
                ...
                return `<main class="main-content" style="text-align:center"><h2>النتيجة: ${score}%</h2><p>صح: ${this.quizScore} من ${this.quizQuestions.length}</p><button class="hero-btn" data-action="goHome">خروج</button></main>`;
            }
            const q = this.quizQuestions[this.quizIndex];
            return `<main class="main-content">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    <button class="hero-btn" data-action="setPage" data-param="reading">✕ إغلاق</button>
        ...
        if (this.currentPage === 'flashcards') {
            ...
            if (!active.length) return `<main class="main-content" style="text-align:center;"><h2>🎉 انتهت البطاقات!</h2><button class="hero-btn" data-action="resetReview" data-param='${JSON.stringify(allTerms.map(t=>t.id))}'>إعادة مراجعة الدرس</button></main>`;
            ...
            return `<main class="main-content">
                ...
                <div style="display:flex; gap:10px;"><button class="hero-btn" data-action="masterWord" data-param="${t.id}" style="background:#059669; flex:1;">✅ حفظتها</button><button class="hero-btn" data-action="deleteWord" data-param="${t.id}" style="background:#dc2626; flex:1;">🗑️ حذف من هنا</button></div>
        ...
        if (this.currentPage === 'addLesson') {
            return `<main class="main-content"><button class="hero-btn" data-action="goHome">← رجوع</button><div class="reading-card" style="margin-top:15px;">
                ...
                <button class="hero-btn" onclick="document.getElementById('camInput').click()" style="width:100%; background:#8b5cf6;">📸 فتح الكاميرا للمسح</button>
                <textarea id="ocrText" placeholder="النص سيظهر هنا..." style="width:100%; height:200px; margin-top:10px; padding:10px; border-radius:10px; border:1px solid #ccc;"></textarea>
